...
void TextElement::setGlobalDisplay(LGFX& display) {
  globalDisplay = &display;
}

TextElement::TextElement() = default;

TextElement::TextElement(int x, int y, uint16_t color, String content, int screen,
...
void TextElement::updateFontPreserveSize(const std::string &baseFontName) {
  if (!allowFontChange) return;
  fontString = baseFontName;
  font = getFontFromNameAndSize(fontString, size9pt);
}

const lgfx::IFont* TextElement::getFont() const {
  return font;
}

const lgfx::IFont* TextElement::getFontFromNameDefault(const std::string &baseFontName) {
  return getFontFromNameAndSize(baseFontName, true);
}

const lgfx::IFont* TextElement::getFontFromNameAndSize(const std::string &baseFontName, bool size9pt) {
...
const lgfx::IFont* TextElement::getFontFromName(const std::string &fontName) {
