...
public:
    ...
    // Method 1: Text element with direct font pointer
    ...
    // Method 2: Text element with font string and size
    static bool createTextElement(const std::string &key, int x, int y, uint16_t color, String content,
                                  int screen, const std::string &fontString, bool size9pt);

    // Method 3: Explicitly control font change behavior
    // To avoid conflict, use a different parameter name for the boolean
    static bool createTextElementWithFontControl(const std::string &key, int x, int y, uint16_t color, String content,
                                  int screen, const std::string &fontString, bool size9pt, bool allowFontChange);

    // Method 4: Rotated text with font pointer
